...
@pytest.mark.parametrize(
    "relative",
    ["data/personas.parquet", "provenance/evidence.json"],
    ids=["output", "evidence"],
)
...
@pytest.mark.parametrize(
    "kind",
    ["symlink", "hardlink", "case", "traversal"],
    ids=["symlink", "hardlink", "case-collision", "traversal"],
)
...
@pytest.mark.parametrize(
    "mutation",
    ["missing", "extra", "empty", "title-map-missing", "title-map-extra"],
    ids=["missing", "extra", "empty", "title-map-missing", "title-map-extra"],
)
...
@pytest.mark.parametrize(
    "expected",
    ["", "not-a-digest", "a" * 63, "g" * 64],
    ids=["empty", "malformed", "short", "non-hex"],
)
