...
def test_freeze_service_is_deterministic_and_bounds_size(tmp_path: Path) -> None:
    ...
    freeze_sample(run_dir=paths["sample"].parent, rows=2, output=first)
    freeze_sample(run_dir=paths["sample"].parent, rows=2, output=second)
    ...
    with pytest.raises(SampleSizeError, match="exceeds the run row count"):
        freeze_sample(run_dir=paths["sample"].parent, rows=3, output=first)
