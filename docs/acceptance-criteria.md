...
- Library and dataset/release validation require the input checksum and successful
  ...
  remain, while generated-content semantic and final-run gates are skipped. Its typed
  policy is bound into the generation context, manifest, checkpoints, and run ID, so it
  cannot collide with or resume guarded artefacts. The direct command is unsafe for
  release evidence. The upstream sampler schema must be version 8 and the frozen-sample
