...
| Rare-cell reconstruction                    | Bundle-bound minimum source count, including FOLK2 origin eligibility, release-size pooling, and 99% coverage gate                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Aggregate combinations may still appear distinctive                                                                                                           |
...
| Synthetic biography mistaken for source data | The generation-5 persona preserves source-backed broad facts while prompts explicitly define detailed education, workplace setting, relationships, family, interests, and ambitions as synthetic elaboration. Person and partner names, real employers or institutions, exact addresses, former work, and appearance remain prohibited. | Readers may still mistake plausible fictional details for observed Statistics Denmark data, so dataset documentation and blinded review remain mandatory |
| Visual or image-model stereotyping          | `visual_persona` is not part of the active schema. The generation-5 persona is not a visual description, but downstream image models may infer or stereotype from ordinary text or origin labels                                                                                                                                                                                                                                                                                                                                                                   | Image generation is outside this contract and requires separate human review                                                                                  |
| Unauthorised LLM execution                  | The Hydra config defaults to a local endpoint; invocations have row and HTTP-request caps, and credentials come from an environment variable. Executable scripts load the repository-root `.env` without overriding existing process values and never log values                                                                                                                                                                                                                         | A user with repository and provider access can deliberately enable bounded calls                                                                              |
...
| Checksum-tolerant, schema-only single-persona command | `generate_persona.py` alone may continue after stored checksum mismatches and deliberately skips generated-content semantic gates. It retains strict `GeneratedPersona` parsing plus upstream, provenance, and request-budget checks. Its typed policy is bound into manifests, checkpoints, and run IDs; dataset, pilot, release, and default library paths remain guarded | Altered upstream artefacts and unsafe generated text can reach provider requests or local output. Direct output is unsafe for release evidence and requires separate review |
...
The default same-sex-partner probability is `0.00701`, calculated from the 2026
Statistics Denmark StatBank FAM100N share among legally formalised couples only:
(PARS 10,180 + RP 4,376) / (PARS + RP + PARF 2,060,670) =
14,556 / 2,075,226. This formalised-couple proxy is applied to all generated
partnered personas. DST cannot identify most unmarried same-sex couples, so this is
not an estimate of the all-partnership rate. It is not sexual orientation or observed
individual data. Source: <https://www.statistikbanken.dk/FAM100N>.

FOLK2's complete official IELAND mapping is retained verbatim as an audit artefact,
including Stateless, Not stated, zero-count rows, and positive sub-threshold rows. Only
categories meeting the bundle-bound minimum source count of 50 are eligible for the
independent national sampling marginal; their weights are renormalised, and the frozen
sample preserves that eligible marginal with deterministic finite quotas. The English
`origin_country` is retained as official source/audit provenance. Mandatory
`origin_country_da` is resolved from the archived official FOLK2 metadata-da 241-code
contract, source metadata SHA-256
