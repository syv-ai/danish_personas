...
| Unsupported persona claims                  | The generation-4 persona must preserve pronoun, age, municipality, Danish origin, broad education, and synthetic title or canonical current status without requiring verbatim clauses. It permits natural elaboration while rejecting pronoun drift, generic subject substitutes, banned contract wording, data-model jargon, and unsupported family claims. | A fluent model may still invent family, work-history, or appearance details, so blinded review remains mandatory |
...
The pooled secondary/vocational education clause is a temporary source-backed broad
level. It must not imply a specific qualification or institution and remains in use
only pending a separate Statistics Denmark detailed-education source.
